...
public class Member {
    ...
    public void setCustomer(Customer customer){
        ...
        //   이 현재 회원 Member가 이 쇼핑몰에서 기존에 자신의 Customer 정보를 가지고 있는 상태라는 것을 의미하므로,
        //   (= 현재 특정된 회원 Member가 이미 어떤 다른 Customer 객체와 연결되어 있는 상태인 경우 라는 의미임)
        //- 'this.customer.setMember(null)'
        //  : 현재 특정된 회원 Member의 기존 Customer 정보를 지워버린다.
        //    즉, 현재 특정된 회원 Member가 가지고 있었던 기존 Customer 정보를
        //    현재 특정된 회원 Member에서 떼어내어 제거시키는 것임.
        //    즉, 현재 Member와 기존 Customer 객체 간의 연관관계 매핑을 끊어버리고 해제시키는 것임.
        //   (= 현재 특정된 회원 Member와 매핑되어 있는 Customer 객체의 회원 Member 정보를 null로 해버리는 것임)
        if(this.customer != null){
            this.customer.setMember(null);
        }

        // Step 2)
        //- 현재 특정된 회원 Member의 비어 있는 판매자 Customer 정보 칸에, 이제 자신이 입력한
        //  고객인 자신의 고객 정보 Customer 정보를 자신인 현재 특정된 회원 Member의 정보로 주입시켜서,
        //  이제 이 회원 Member가 고객으로서의 자신만의 Customer 정보를 갖도록 한다.
        this.customer = customer;


        // Step 3)
        //- 'customer != null'
        //  : 혹시라도 만약에, 이 회원 Member가 입력한 고객으로서의 자신의 Customer 정보(=외부에서 새롭게 들어온 Customer 정보)가
        //    비어 있는 상태가 아니면서(='&&'),
        //    (그냥 의례적으로 NullPointerException을 방지하기 위해 입력한 부분.
        //     당연히, 외부에서 새롭게 들어온 Customer 정보가 비어 있지 않은 상태여야
        ...
        //  : 자신이 입력한 정보에 이미 다른 회원 Membmer 정보가 들어있던(붙어있던) 상황이라면
        //    (=다른 회원 Member 정보와 매핑되어 있는 상황이라면),
        //    (=외부에서 새롭게 들어온 Customer 정보가 이미 다른 Member를 참조하고 있는 상황이라면)
        //- 'customer.setMember(this)'
        //  : 그 다른 회원 Member 정보를 자신이 입력한 고객 Customer 정보에서 없애고 외부에서 새롭게 들어온
        //    고객으로서의 자신 Customer 정보로 대체해야 하기 때문에, 그 방법으로
    ...
    //< Member(1) : Seller (1). 1:1 양방향 매핑. 연관관계 편의 메소드 >
    //- 연관관계 편의 메소드는 기본적으로 Setter 세터와 형식이 같음!!
    public void setSeller(Seller seller){

        // Step 1)
        //- 'this.seller != null'
        //  : 만약, 현재 특정된 회원 Member의 파냄자 Seller 정보가 비어져 있지 않은 상태(=깨끗하지 않은)이라면,
        ...
        //   (= 현재 특정된 회원 Member와 매핑되어 있는 Seller 객체의 회원 Member 정보를 null로 해버리는 것임)
        if(this.seller != null){
            this.seller.setMember(null);
        }
        ...
        // Step 3)
        //- seller != null
        //  : 혹시라도 만약에, 외부에서 새롭게 들어온 이 회원 Member가 입력한 판매자로서의 자신의 Seller 정보가
        //    비어 있는 상태가 아니면서(='&&')
        //- seller.getMember() != this
        //  : 자신이 입력한 seller 정보에 이미 다른 회원 Membmer 정보가 들어있던(붙어있던) 상황이라면
        //    (=다른 회원 Member 정보와 매핑되어 있는 상황이라면),
        //- seller.setMember(this)
        //  : 그 다른 회원 Member 정보를 자신이 입력한 판매자 Seller 정보에서 떼어내어 제거시켜서
